    ...
    jqPopup.dialog({
      ...
      handler: function (event) {
        if (event === "before-hide") {
          ...
          let foundItemNo = that.indexOfText(selItemText);

          if (foundItemNo >= 0) {
            that.setSelectedItem(foundItemNo, null, true);
            return;
          }
          ...
          if (isInsert) {
            ...
            if (!that.parseItem(selItemNo, selItemText)) {
              return;
            }
            _jqItems.find("[selected]").text(that.formatItem(selItemNo));
          }
        }
