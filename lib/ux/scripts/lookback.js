...
class LookBack {
  ...
  constructor(gradient, imageUrl, alignment) {
    if ((gradient instanceof Object) && gradient.gradient) {
      let from = gradient;
      this.init(from.gradient, from.imageUrl, from.alignment)
    } else {
      this.init(gradient, imageUrl, alignment);
    }
  }
  ...
  init(gradient, imageUrl, alignment) {
  ...
  toStyle() {
    ...
    if (this.gradient) {
      result.append(this.gradient.toStyle());
    }
    ...
    if (this.imageUrl) {
      ...
      if (this.alignment) {
        result.append(this.alignment.toBackStyle());
      }
    }
