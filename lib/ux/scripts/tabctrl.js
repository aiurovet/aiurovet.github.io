...
$.fn.tabctrl = function(options) {
  ...
  this.setBounds = function(jqFrom) {
    const isFrom = jqFrom && (jqFrom.length >= 0) ? true : false;
    let maxHeight = isFrom ? jqFrom.maxHeight : 0;
    let maxWidth = isFrom ? jqFrom.maxWidth : 0;
  ...
  this.setSelectedItem = function(itemNo) {
    ...
    let jqActiveTab = null;

    _jqItemContents.each(function (index) {
      let jqThis = $(this);
      jqThis.setVisible(index === itemNo);
    ...
    if (jqActiveTab && !getFocus()) {
      jqActiveTab.getFirstFocusableChild(false)?.postFocus();
    }
  }
