...
$.fn.spinner = function(options) {
  ...
  this.getInputValue = function() {
    return this.isList ? _jqValue.text() : this.parseValue(_jqValue.val());
  }
  ...
  this.initData = function(options) {
    ...
    if (this.isList) {
      ...
      if (this.isWrap && ((this.min === null) || (this.max === null))) {
        this.isWrap = false;
      }
  ...
  this.initUI = function(options) {
    ...
    let readonly = this.isReadOnly ? "readonly" : "";
    ...
    _jqValue
      .on("change", function(event) {that.onEdit(event);});
  ...
  this.onEdit = function(event) {
    ...
    value = hasValue ? this.parseValue(value) : this.getInputValue();
  ...
  this.parseValue = function(text) {
    return this.parser
      ? this.parser(text)
      : this.isList ? text : this.isInteger ? parseInt(text) : parseFloat(text);
  }
  ...
  this.update = function(value, step) {
    let newData = (this.isList ? this.index : (value ?? this.getInputValue() ?? 0)) + (step ?? 0);
