...
class Gradient {
  ...
  constructor(fromColor, fromRatio, direction, toColor) {
    if (fromColor instanceof Object) {
      let from = fromColor;
      this.init(from.fromColor, from.fromRatio, from.direction, from.toColor)
    } else {
      this.init(fromColor, fromRatio, direction, toColor);
    }
  }
  ...
  init(fromColor, fromRatio, direction, toColor) {
  ...
  toStyle() {
    ...
    const fromColor = fromRatioPc <= 0 || fromRatioPc >= 100 ? this.fromColor : `${this.fromColor} ${fromRatioPc}%`;

    if (fromRatioPc >= 100) {
      result["background-color"] = fromColor;
    } else {
      ...
      if (direction === Direction.radial) {
        result["background-color"] = `radial-gradient(${fromColor}, ${this.toColor})`;
      } else {
        const angle =
        ...
        result["background-color"] = `linear-gradient(${angle}deg, ${fromColor}, ${this.toColor})`;
      }
    }
