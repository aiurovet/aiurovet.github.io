...
$.fn.listedit = function(options) {
  ...
  this.initUI = function(options) {
    ...
    if ((options === undefined) || (options === null)) {
      _jqItems.find(".ui-listedit-item").off();
      _jqItems = null;
    ...
    _jqItems.find(".ui-listedit-item[selected]").postFocus();
  ...
  this.onClickModify = function(isInsert) {
    var [selItemNo, selItemText, editorContent] = this.setContent(isInsert);
  ...
  this.onModify = function(event, jqPopup, isInsert, selItemNo, selItemText) {
    if (event === "before-show") {
      const title = (isInsert ? this.insertTitle : this.modifyTitle);
      jqPopup.find(".ui-dialog-caption-text").html(title.toHtml());
      jqPopup.find(".ui-listedit-modify").val(selItemText);
      ...
      if (foundItemNo >= 0) {
      ...
      } else if (((selItemText?.length ?? 0) > 0)) {
        if (isInsert) {
        ...
        } else if (this.parseItem(selItemNo, selItemText)) {
    ...
    if (this.editorHandler) {
      this.editorHandler(this, event, isInsert, selItemNo, selItemText);
    }
  }
  ...
  this.setContent = function(isInsert) {
    ...
    if (!editorContent) {
      const inputHtml = this.editorRows > 1
        ? `<textarea class="ui-listedit-modify" rows="${this.editorRows}"></textarea>`
        : `<input class="ui-listedit-modify" type="text">`;
