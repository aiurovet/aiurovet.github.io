...
$.fn.listedit = function(options) {
  ...
  this.onClickModify = function(isInsert) {
    ...
    var selItemNo = isInsert ? this.items.length : this.selectedItemNo;
    var selItemText = isInsert ? "" : this.formatItem(selItemNo);

    if (this.onModify) {
      this.onModify(isInsert, jqPopup, selItemNo, selItemText);
      return;
    }

    const editorHtml = this.editorRows > 1
    ...
    jqPopup.dialog({
      ...
      handler: function (event) {
        that.onModifyDefault(isInsert, jqPopup, selItemNo, selItemText, event);
      }
    });
  }
  ...
  this.onModifyDefault = function(isInsert, jqParentDialog, selItemNo, selItemText, event) {
    if (event === "before-hide") {
      selItemText = jqParentDialog.find(".ui-listedit-modify").val().trim();
