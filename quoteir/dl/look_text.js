...
class LookText {
  ...
  constructor(color, font, padding, text) {
    ...
    if (color instanceof Object) {
      let from = color;
      this.init(from.color, from.font, from.padding, from.text)
    } else {
      this.init(color, font, padding, text);
    }
  }
  ...
  toStyle() {
    ...
    if (effects && effects.contains(LookText.bold)) {
      result["font-weight"] = "bold";
    }
    if (effects && effects.contains(LookText.italic)) {
      result["font-style"] = "italic";
    }
    if (effects && effects.contains(LookText.underline)) {
